...
static int nn_(SpatialDeconvolution_updateOutput)(lua_State *L)
{
...

  {
    ...
    long outputWidth = (inputWidth - 1) *  dW + kW;  //where is padding ?
...
#pragma omp parallel for private(k)
    ...
    //THTensor_(conv2Dger)(gradWeight, 1.0, scale, input, gradOutput, dH, dW, 'F', 'C');//? scale ? for what?
    THTensor_(conv2DRevger)(gradWeight, 1.0, scale, gradOutput, input, dH, dW);//caffe: change the orders of bottom data & top_diff
  }
  else
  {
    long k;
#pragma omp parallel for private(k)
    ...
    THTensor_(conv2DRevgerm)(gradWeight, 1.0, scale, gradOutput, input, dH, dW);//caffe: change the orders of bottom data & top_diff
    // THTensor_(conv2Dger)(gradWeight, 1.0, scale, input, gradOutput,dH, dW, 'F', 'C');
    //THTensor_(conv3Dger)(gradWeight, 1.0, scale, input, gradOutput, 1,dH, dW, 'F', 'C');
